...
class DataReader:
    def __init__(
        ...
        def handle_file_extension(fe: str):
        ...
        if isinstance(file, str):
            ...
            handle_file_extension(file_extension)
    ...
    def _read(self) -> Tuple[Union[pd.DataFrame, List, Dict], Iterable]:
        ...
        if not self.is_dir:
            if self.file_extension == 'csv':
                ...
                return read_xml(file_contents:=self.file), [file_contents]
            else:
            ...
            #                      f": {self.file}.")
            raise NotImplementedError("Reading from a directory is not yet implemented.") 
